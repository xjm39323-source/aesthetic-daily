...
async function loadData() {
  if (typeof INLINE_DATA !== "undefined" && INLINE_DATA && INLINE_DATA.theme && !state.theme) {
    state.theme = INLINE_DATA.theme; render(); return
  }
  try {
...
function renderWork() {
  const t = state.theme
  if (!t) return
  const w = t.works[state.workIndex]
  if (!w) return

  const fav = Store.isFavorited(w.id || w.imageName)
  document.getElementById("work-detail").innerHTML = `
    ${w.imageName ? `<img class="work-image" src="${w.imageName}" alt="${w.title}" loading="lazy" onerror="this.style.background='var(--light)';this.alt=''" />`
      : `<div class="work-image" style="display:flex;align-items:center;justify-content:center;color:var(--text-secondary)">✦</div>`}
    <div class="work-info">
      <div>
...
function nextWork() {
  const t = state.theme
  if (t && state.workIndex < t.works.length - 1) {
    state.workIndex++
    renderWork()
  }
}
...
function toggleFav(id) {
  const isNow = Store.toggleFavorite(id)
  Store._set("favs", Store.getFavorites()) // persist
  renderWork()
}
...
function renderExplore() {
  ...
  document.getElementById("explore-grid").innerHTML = cats.map(c =>
    `<div class="cat-card"><div class="icon">${icons[c]||"✦"}</div><div class="name">${c}</div></div>`
  ).join("")
}
...
function renderFavorites() {
  ...
  let html = ""
  if (!favIds.length) {
    html = `<div class="empty-state"><div class="icon">♡</div><p>还没有收藏</p></div>`
  } else {
    html = favIds.map(id => {
      ...
      return `<div style="display:flex;align-items:center;gap:12px;padding:12px 16px;background:var(--card);border-radius:8px;margin-bottom:8px;border:1px solid var(--border)">
        ${w && w.imageName ? `<img src="${w.imageName}" style="width:60px;height:60px;border-radius:8px;object-fit:cover" />`
        ...
        <div>
          <div style="font-size:14px;color:var(--text-title)">${w ? w.title : id}</div>
          <div style="font-size:11px;color:var(--text-secondary)">已收藏</div>
        </div>
      </div>`
    }).join("")
  }
...
function renderProfile() {
  const days = Store.getDaysSinceJoin()
  const favCount = Store.getFavorites().length
  const completedCount = Store.getCompletedCount()
  document.getElementById("profile-content").innerHTML = `
    <div class="profile-header">
      <div class="avatar">✦</div>
      <div class="profile-name">${Store.getProfile().name}</div>
      <div class="profile-days">加入第 ${days} 天</div>
    </div>
    <div class="stats">
    ...
    <div class="section">
      <div class="label">审美偏好</div>
      <div class="pref-card" id="pref-bars"></div>
    </div>
  `
  const cats = ["电影美学", "三维/CG", "摄影", "动漫", "平面设计", "建筑/空间", "插画", "时尚", "UI/UX", "自然/微距"]
  document.getElementById("pref-bars").innerHTML = cats.map(c =>
    `<div class="pref-row">
      <div class="row"><span>${c}</span><span>10%</span></div>
      <div class="pref-bar"><div class="fill" style="width:10%"></div></div>
    </div>`
  ).join("")
}
